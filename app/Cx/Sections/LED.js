      ...
      try {
        ...
        const normalized = (Array.isArray(data) ? data : []).map((item) => {
          ...
          const computedDescription =
            rawDescription ||
            [item.features, item.series].filter(Boolean).join(' • ') ||
            computedName;
          ...
          return {
            ...
            cartId: rawId ? `accessory-${rawId}` : undefined,
            price: parsedPrice,
            hasPrice,
            priceDisplay,
    ...
    <div className={`w-full py-8 lg:py-12 bg-white ${openSans.className}`}>
      <div className="max-w-7xl mx-auto px-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-3xl font-bold text-gray-900">Shop Accessories</h2>
        </div>
          ...
          >
            {loading ? (
              ...
              displayProducts.map((product) => (
                <AccessoryCard key={product.cartId || `accessory-${product.id}`} product={product} />
              ))
            )}
          </div>
